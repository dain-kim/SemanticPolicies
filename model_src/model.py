...
class PolicyTranslationModel(tf.keras.Model):
    ...
    @tf.function
    def get_attention(self, language_tf, features_tf, training=False):
        # print('get attention running on graph mode? ', not tf.executing_eagerly())
        sentence_embedding  = self.embedding(language_tf)
        sentence_embedding  = self.lng_gru(inputs=sentence_embedding, training=training) 
        return (sentence_embedding, self.attention((sentence_embedding, features_tf)))
